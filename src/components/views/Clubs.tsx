...
const TheClubs: React.FC = () => {
    ...
    const getClubs = async () => {
        axios.get('https://app.vzpieranie.sk:3002/api/club').then(response => {
    ...
    const getSortedClubs = async (key: string) => {
        ...
        axios.get(`https://app.vzpieranie.sk:3002/api/club?sortBy=${key}`).then(response => {
            if(Array.isArray(response.data)){
                setClubs(response.data);
            } else {
                setClubs({ message: "Nenašli sa žiadne kluby"});
    ...
    const createClub = async (club: CreateClub) => {
        ...
        axios.post('https://app.vzpieranie.sk:3002/api/club', club).then(async response => {            
    ...
    const editClub = async (club: EditClub) => {
        ...
        axios.patch(`https://app.vzpieranie.sk:3002/api/club/${club.id}`, club).then(async response => {
            const code = response.status;
            if(code === 201) {
                setAlert({
                    alertType: true,
    ...
    const deleteClub = async (id: number) => {
        ...
        axios.delete(`https://app.vzpieranie.sk:3002/api/club/${id}`).then(response => {
            const code = response.status;
            if(code === 200) {                
                setAlert({
                    alertType: true,
